...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleError { msg: "Title cannot be empty".to_string() });
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleError { msg:"Title cannot be longer than 50 bytes".to_string()});
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionError { msg: "Description cannot be empty".to_string()});
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError { msg:"Description cannot be longer than 500 bytes".to_string()});
        }
